...
- 在各自平台后台旋转 / 删除本轮暴露过的旧 key：
  - `OpenRouter`
  - `Tencent Coding Plan`
- 基于“当前 Hermes 应用模板 + 飞书接管已验证”的现网状态，再制作一次新的 CVM 快照 / 镜像
- 记录当前飞书应用 `灵犀-Hermes-Tencent` 为现网主入口，避免后续误切回旧应用

### 17.2 建议短期观察

- 观察 `hermes-gateway.service` 是否持续为 `active`
- 观察飞书对话是否仍能稳定回复
- 观察是否再出现 provider 配置告警升级为实际故障

### 17.3 可延后处理

- 规范化 `model.provider` 与 provider 配置，尽量消除 `doctor` 中的兼容性告警
- 如旧飞书应用已不再使用，可在完成观察期后下线或归档
- 如需标准化重建路径，可单独规划 “Hermes 黄金镜像” 制作与验证
